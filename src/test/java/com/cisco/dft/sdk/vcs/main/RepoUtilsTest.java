...
public class RepoUtilsTest {
	...
	public void testApp() throws GitAPIException, SVNException {
		...
		Application
				.setConfiguration(ProgramConfig.parseArgs("help", "analyze"))
				.execute();

		LOGGER.debug("Testing force run as SVN");
		Application.setConfiguration(
				ProgramConfig.parseArgs("analyze",
						"https://github.com/Himself12794/powersAPI.git", "-s",
						"-d")).execute();
	...
	public void testRepoStatGathering() throws Exception {
		...
		HistoryViewer bi = (HistoryViewer) reo.getRepoStatistics()
				.getBranchInfoFor("master");
		HistoryViewer hv = reo.getRepoStatistics().getBranchInfoFor("master")
		...
		assertTrue("8d7abf9d55a6170af465dce7887c4f399d31a7ba".equals(hv
				.getLastCommitId()));
		LOGGER.info("Uses CLOC statistics: "
				+ String.valueOf(hv.usesCLOCStats()));
		assertTrue(hv.usesCLOCStats() ? hv.getFileCount() == 23 : hv
				.getFileCount() == 37);
		assertTrue(hv.usesCLOCStats() ? hv.getLineCount() == 2499 : hv
				.getLineCount() == 5342);
		assertTrue("master".equals(hv.getBranchName()));
		assertTrue(hv.usesCLOCStats() ? hv.getLangStats(Language.JAVA)
				.getnFiles() == 23
				: hv.getLangStats(Language.JAVA).getnFiles() == 23);
	...
	public void testAuthorStatGathering() throws Exception {
		...
		AuthorInfoBuilder aib = reo.getRepoStatistics()
				.getBranchInfoFor("master").getAuthorStatistics();
		...
		assertTrue(aib.getInfo().get(0).getAdditions() >= aib.getInfo().get(1)
				.getAdditions());

		aib.sort(SortMethod.COMMITS);
		assertTrue(aib.getInfo().get(0).getCommitCount() >= aib.getInfo()
				.get(1).getCommitCount());

		aib.sort(SortMethod.DELETIONS);
		assertTrue(aib.getInfo().get(0).getDeletions() >= aib.getInfo().get(1)
				.getDeletions());

		aib.sort(SortMethod.NAME);
		assertTrue(aib.getInfo().get(0).getName()
				.compareTo(aib.getInfo().get(1).getName()) < 0);

		LOGGER.info("Testing commit info accuracy");
		List<AuthorCommit> commits = ai.getCommits();
		AuthorCommit ac = ai
				.getCommitById("f9b9131491db5110f4dbc839f40b94b0f28fcf85");
		...
		assertTrue(ac.getMessage().equals(
				"Merge pull request #29 from RichardBronosky/master"));
